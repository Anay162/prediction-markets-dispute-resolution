...
from api.schemas.report import ReportOutput, Severity, VulnerabilityCategory
...
async def test_submit_audit_returns_202_with_job_id(async_client):
    with patch("api.routers.audit.job_set_status", new_callable=AsyncMock), \
...
async def test_get_status_pending_job(async_client):
    ...
    with patch("api.routers.audit.job_get_status", new_callable=AsyncMock,
               return_value=status_data), \
         patch("api.routers.audit.get_report_by_job", new_callable=AsyncMock,
               return_value=None):
...
async def test_list_contracts_returns_list(async_client):
    with patch("api.routers.contracts.list_contracts", new_callable=AsyncMock,
               return_value=[]):
...
async def test_get_contract_not_found(async_client):
    with patch("api.routers.contracts.get_contract", new_callable=AsyncMock,
               return_value=None):
...
async def test_get_report_not_found(async_client):
    with patch("api.routers.reports.get_report", new_callable=AsyncMock,
               return_value=None):
