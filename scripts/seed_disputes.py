...
async def main(platform: str | None, limit: int, skip_embed: bool) -> None:
    ...
    async with get_session() as db:
        ...
        result = await db.execute(
            select(DisputeRecord).where(DisputeRecord.embedding == None).limit(2000)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--platform",
        choices=["polymarket", "uma", "manifold", "augur"],
        default=None,
        help="Scrape only one platform (default: all)",
    )
    parser.add_argument(
        "--limit", type=int, default=500, help="Max records per scraper (default: 500)"
    )
